...
public class PasswordManagementUtils {
	...
	protected byte[] generateEncryptedPassword(String enteredPassword, byte[] salt) 
			throws GeneralSecurityException {
		/* Strings are immutatable, so there is no way to change/nullify/modify its content after use.
		So always, collect and store security sensitive information in a char array instead.*/ 
		...
		} catch(NullPointerException npe) {throw new GeneralSecurityException("Salt " + returnStringRep(salt)
		+ "is null") ;}
		catch(IllegalArgumentException iae) {throw new GeneralSecurityException("One of the argument is "
				+ "illegal. Salt " + returnStringRep(salt) + " may be of 0 length, iteration count " + 
				ITERATION_COUNT + " is not positive or derived key length " + DERIVED_KEY_LENGTH + 
				" is not positive." ) ;}
